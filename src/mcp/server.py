...
class LLMEvaluationGateway:
    ...
    """
    def __init__(self, *, name: str = "Demo-MCP", title: str = "Demo MCP (fastapi_mcp)", description: str = "Demo server: FastAPI endpoints automatically exposed as MCP tools.", version: str = "0.1.0", protect_mcp_with_token: bool = True) -> None:
    ...
    def _simulate_kpi_fetch(req: KPIRequest) -> Dict[str, Any]:
        ...
        key = (
            req.system.lower().strip(),
            req.kpi_name.lower().strip(),
            (req.region or "eu").lower().strip(),
        )
        value = db.get(key)
